...
contract HandsOnNFT_Auction is AccessControl, IERC721Receiver, IERC1155Receiver {
    ...
    function createAuctionSales( address _token, uint256 _tokenId, uint256 _tokenERCType, uint256 _amountOfToken, uint256 _startPrice, uint256 _minIncrement, uint256 _startAt, uint256 _endAt ) external onlyRole(AUCTION_LISTER_ROLE) returns (bool) {
        require(_amountOfToken > 0, "createAuctionSales: The amount of tokens to sell, needs to be greater than 0");
        require(_startPrice > 0, "createAuctionSales: The startPrice for the tokens need to be greater than 0");
        //테스트환경이라 주석처리 require(_endAt - _startAt > 86400, "createAuctionSales: The endAt should to be greater than 1 day from startAt");
        require(_startPrice > 0, "createAuctionSales: The start Price should be bigger than 0");
        require(_minIncrement > 0, "createAuctionSales: The minIncrement should be bigger than 0");
        //정책 정하고서 진행필요. require(_startAt > block.timestamp, "createAuctionSales: The start date should be after now");
        require(_tokenERCType == 721 || _tokenERCType == 1155, "createAuctionSales: Not supported token type");

        // Auction Contract가 경매 진행할 토큰을 수령 후 경매 진행. (받지않고 경매하면 경매낙찰 시 원 소유주가 빼돌린 이후일 경우가 존재할 수 있음)
        if(_tokenERCType == 721) {
            require(_amountOfToken == 1, "createAuctionSales: ERC721 amountOfToken must be 1");
    ...
    function placeBid( uint256 _auctionDataId ) external payable returns (bool) {
        ...
        require(msg.value >= auctionData.highestBid + auctionData.minIncrement, "placeBid: Bid amount should be bigger than highestBid");
        require(msg.sender != auctionData.seller, "placeBid: seller can't bid");
        require(block.timestamp >= auctionData.startAt, "placeBid: Bid should be after the startAt");
        require(auctionData.endAt > block.timestamp, "placeBid: It is Ended");
    ...
    function cancelAuction( uint256 _auctionDataId ) external returns (bool) {
        ...
        require(msg.sender == auctionData.seller, "cancelAuction: Only auction seller can cancel it");
        require(block.timestamp > auctionData.endAt, "cancelAuction: The time should be after endDate");
        require(auctionData.highestBidder == address(0), "cancelAuction: There should be not highestBidder");
    ...
    function claimAuction( uint256 _auctionDataId ) external returns (bool) {
        ...
        require(msg.sender == auctionData.highestBidder, "claimAuction: The msg.sender should be the highest Bidder");
        require(block.timestamp > auctionData.endAt, "claimAuction: The time should be after endDate");
